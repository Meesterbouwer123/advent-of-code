...
res = 0
for set in [horizontals, verticals, [item for (_, item) in diagonals1.items()], [item for (_, item) in diagonals2.items()]]:
    res += sum([line.count('XMAS') for line in set])
    res += sum([line.count('SAMX') for line in set])
